...
//             toast.warning("Please select both contracts to compare.");
//             return;
//         }
//         if (originalId === revisedId) {
...
//         .diff-root { font-family: 'Sora', sans-serif; min-height: 100vh; background: #050816; color: #F1F5F9; padding: 2.5rem 1.25rem; }
//         .diff-wrap { max-width: 900px; margin: 0 auto; position: relative; z-index: 1; }
//         .diff-card { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.09); border-radius: 20px; padding: 1.5rem; margin-bottom: 1.5rem; backdrop-filter: blur(10px); }
//         .diff-select { width: 100%; padding: 12px; background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); color: #fff; border-radius: 10px; outline: none; margin-top: 8px; font-family: 'DM Sans', sans-serif; }
//         .diff-select:focus { border-color: #C8A850; }
//         .diff-btn { background: linear-gradient(135deg, #C8A850, #a07830); color: #0F172A; border: none; padding: 12px 24px; border-radius: 12px; font-weight: 700; cursor: pointer; transition: transform 0.2s; width: 100%; font-size: 1rem; }
//         .diff-btn:hover { transform: translateY(-2px); }
//         .diff-table { width: 100%; border-collapse: collapse; margin-top: 1rem; }
//         .diff-th { text-align: left; padding: 12px; border-bottom: 1px solid rgba(255,255,255,0.1); color: #94A3B8; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; }
//         .diff-td { padding: 14px 12px; border-bottom: 1px solid rgba(255,255,255,0.05); font-family: 'DM Sans', sans-serif; }
//         .diff-chip { display: inline-block; padding: 4px 12px; border-radius: 999px; font-size: 0.75rem; font-weight: 700; margin: 4px; }
//       `}</style>
...
//                         <div>
//                             <span style={{ background: 'rgba(200,168,80,0.15)', color: '#C8A850', padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 700 }}>SMART RE-ANALYSIS</span>
//                             <h1 style={{ margin: '8px 0 0', fontSize: '2rem', fontWeight: 800 }}>Contract Diff</h1>
//                         </div>
//                         <Link to="/" style={{ color: '#94A3B8', textDecoration: 'none', border: '1px solid rgba(255,255,255,0.1)', padding: '8px 16px', borderRadius: '10px' }}>Dashboard</Link>
//                     </div>

//                     <div className="diff-card" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
//                         <div>
//                             <label style={{ fontSize: '0.85rem', color: '#94A3B8', fontWeight: 600 }}>Original Contract (Before)</label>
//                             <select className="diff-select" value={originalId} onChange={e => setOriginalId(e.target.value)}>
//                                 <option value="">Select original contract...</option>
//                                 {history.map(h => <option key={h._id} value={h._id}>{h.fileName} ({new Date(h.uploadedAt).toLocaleDateString()})</option>)}
//                             </select>
//                         </div>
//                         <div>
//                             <label style={{ fontSize: '0.85rem', color: '#94A3B8', fontWeight: 600 }}>Revised Contract (After)</label>
//                             <select className="diff-select" value={revisedId} onChange={e => setRevisedId(e.target.value)}>
//                                 <option value="">Select revised contract...</option>
//                                 {history.map(h => <option key={h._id} value={h._id}>{h.fileName} ({new Date(h.uploadedAt).toLocaleDateString()})</option>)}
//                             </select>
//                         </div>
//                         <div style={{ gridColumn: '1 / -1', marginTop: '0.5rem' }}>
//                             <button className="diff-btn" onClick={handleCompare} disabled={loading}>
//                                 {loading ? 'Analyzing Changes...' : 'Compare Versions'}
//                             </button>
//                         </div>
//                     </div>

//                     {diffData && (
//                         <div style={{ animation: 'page-enter 0.5s ease' }}>
//                             <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
//                                 <div className="diff-card" style={{ textAlign: 'center', marginBottom: 0 }}>
//                                     <p style={{ color: '#94A3B8', fontSize: '0.8rem', margin: '0 0 10px', textTransform: 'uppercase' }}>Score Change</p>
//                                     <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '15px' }}>
//                                         <span style={{ fontSize: '2rem', fontWeight: 700, color: '#94A3B8' }}>{diffData.priceChange.originalScore || 0}</span>
//                                         <span style={{ fontSize: '1.5rem' }}>➔</span>
//                                         <span style={{ fontSize: '2.5rem', fontWeight: 800, color: diffData.priceChange.revisedScore >= diffData.priceChange.originalScore ? '#10B981' : '#EF4444' }}>
//                                             {diffData.priceChange.revisedScore || 0}
//                                         </span>
//                                     </div>
//                                 </div>
//                                 <div className="diff-card" style={{ marginBottom: 0 }}>
//                                     <p style={{ color: '#94A3B8', fontSize: '0.8rem', margin: '0 0 10px', textTransform: 'uppercase' }}>Fees Activity</p>
//                                     <div>
//                                         {diffData.feesRemoved.length === 0 && diffData.feesAdded.length === 0 && <p style={{ color: '#F1F5F9', fontSize: '0.9rem' }}>No changes to hidden fees.</p>}
//                                         {diffData.feesRemoved.map(f => <span key={f} className="diff-chip" style={{ background: 'rgba(16,185,129,0.15)', color: '#10B981', border: '1px solid rgba(16,185,129,0.3)' }}>✅ Removed: {f}</span>)}
//                                         {diffData.feesAdded.map(f => <span key={f} className="diff-chip" style={{ background: 'rgba(239,68,68,0.15)', color: '#EF4444', border: '1px solid rgba(239,68,68,0.3)' }}>⚠️ Added: {f}</span>)}
//                                     </div>
//                                 </div>
//                             </div>

//                             <div className="diff-card">
//                                 <p style={{ color: '#94A3B8', fontSize: '0.8rem', margin: '0 0 10px', textTransform: 'uppercase', fontWeight: 700 }}>Line-by-Line Changes</p>
//                                 <div style={{ overflowX: 'auto' }}>
//                                     <table className="diff-table">
//                                         <thead>
//                                             <tr>
//                                                 <th className="diff-th">Field</th>
//                                                 <th className="diff-th">Original</th>
//                                                 <th className="diff-th">Revised</th>
//                                                 <th className="diff-th">Status</th>
//                                             </tr>
//                                         </thead>
//                                         <tbody>
//                                             {diffData.fieldDiffs.map((row, i) => (
//                                                 <tr key={i} style={{ background: row.direction === 'improved' ? 'rgba(16,185,129,0.05)' : row.direction === 'worsened' ? 'rgba(239,68,68,0.05)' : 'transparent' }}>
//                                                     <td className="diff-td" style={{ color: '#F1F5F9', fontWeight: 600, textTransform: 'capitalize' }}>{row.field}</td>
//                                                     <td className="diff-td" style={{ color: '#94A3B8' }}>{row.before}</td>
//                                                     <td className="diff-td" style={{ color: '#F1F5F9' }}>{row.after}</td>
//                                                     <td className="diff-td">
//                                                         {row.direction === 'improved' && <span style={{ color: '#10B981', fontWeight: 700 }}>Improved</span>}
//                                                         {row.direction === 'worsened' && <span style={{ color: '#EF4444', fontWeight: 700 }}>Worsened</span>}
//                                                         {row.direction === 'changed' && <span style={{ color: '#C8A850', fontWeight: 700 }}>Changed</span>}
//                                                         {row.direction === 'unchanged' && <span style={{ color: '#64748B' }}>Unchanged</span>}
//                                                     </td>
//                                                 </tr>
//                                             ))}
//                                         </tbody>
...
//                 style={{ padding: '14px 16px', background: isOpen ? 'rgba(200,168,80,0.08)' : 'rgba(0,0,0,0.3)', border: `1px solid ${isOpen ? '#C8A850' : 'rgba(255,255,255,0.1)'}`, borderRadius: '12px', color: selectedOpt ? '#F1F5F9' : '#64748B', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', transition: 'all 0.2s', boxShadow: isOpen ? '0 0 0 3px rgba(200,168,80,0.15)' : 'none' }}
//             >
...
//                                                     <span style={{ fontSize: '1.5rem', color: isBetterOrSame ? '#10B981' : '#EF4444' }}>➔</span>
//                                                     <span style={{
//                                                         fontSize: '3.5rem',
//                                                         fontWeight: 800,
...
const ContractDiffPage = () => {
    ...
    // State & Refs for direct uploading
    ...
    // 🔥 NEW: Check memory on page load. If a diff exists, put it back on screen!
    useEffect(() => {
    ...
    // Handle Direct Upload from the Diff Page
    const handleDirectUpload = async (e, target) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                fetchHistory(); // Refresh the dropdowns
                // Auto-select the file we just uploaded
            ...
            e.target.value = null; // reset the input so same file can be selected again if needed
        }
    };
    ...
    const handleCompare = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                // 🔥 NEW: Save the exact result to the browser's memory!
                ...
                <div className="diff-wrap">
                    ...
                    <div className="diff-card">
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
                            ...
                            {/* Original Contract Selector + Upload */}
                            <div>
                            ...
                            {/* Revised Contract Selector + Upload */}
                            <div>
                                <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
                                    <div style={{ flex: 1 }}>
                                        <CustomSelect
